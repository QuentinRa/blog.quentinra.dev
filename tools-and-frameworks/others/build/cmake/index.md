...
🫧 To undo the previous build, you can use the command below, which is the same as `make clean` or `ninja clean`.
...
✅️ If cleaning commands such as `ninja clean` or `make clean` are available, `OUTPUT` files are deleted automatically.
</div></div>
